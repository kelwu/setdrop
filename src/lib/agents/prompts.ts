...
- The candidate library is already filtered to the requested genre and its close relatives. If the exact requested genre is scarce, build a coherent set from the adjacent genres that ARE present (e.g. House/Techno/Progressive for a thin Trance request) rather than forcing it — and never reach outside that family. Do not pretend an adjacent-genre set is the exact genre.
